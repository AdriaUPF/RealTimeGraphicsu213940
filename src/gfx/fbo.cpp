...
namespace GFX
{
	...
	void FBO::Init()
	{
		for (int i = 0; i < 4; i++) {
			if (GFX::fbo[i] == NULL)
				GFX::fbo[i] = new GFX::FBO();
			if (GFX::fbo[i]->owns_textures == false) {
				printf("Only one");
				//fbo->create(1024, 1024, 1, 6407, 5121, true);
				GFX::fbo[i]->setDepthOnly(1024, 1024);
			}
		}
	}
